...
#    The_Revised_Harris_Benedict_Equation_Men
#    BMR = 88.362 + (13.397 x weight in kg) + (4.799 x height in cm) - (5.677 x age in years)
#    Women_BMR = 447.593 + (9.247 x weight in kg) + (3.098 x height in cm) - (4.330 x age in years)
    def women_bmr(self):
...
class Claries():
    ...
    def cal_men_gian(self):
        ...
        gain=self.bmr-(self.activity*self.time)+(self.target/float(self.target_time))+ (self.activity * self.time)
        return gain
    def cal_women(self):
        self.wbmr = 10 * self.weight + (6.25 * self.height) - (5 * self.age) - 161
        gain = self.wbmr-(self.activity*self.time)+(self.target/float(self.target_time))+ (self.activity * self.time)
        return gain
    def cal_men_loss(self):
        #7,700 Kcal
        self.bmr =10 * self.weight+(6.25*self.height)-5 * self.age +5
        loss=self.bmr-(self.activity*self.time)+(self.target/float(self.target_time))
        return loss
    def cal_women_loss(self):
        self.wbmr = 10 * self.weight + (6.25 * self.height) - (5 * self.age) - 161
        loss = self.wbmr-(self.activity*self.time)+(self.target/float(self.target_time))
        return loss
