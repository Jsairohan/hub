...
class JSONParser(BaseParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        ...
        try:
            decoded_stream = codecs.getreader(encoding)(stream)
            # decoded_streamwrite = codecs.getwriter(encoding)(stream)
            parse_constant = json.strict_constant if self.strict else None
            # print "dede",stream

            j =json.load(decoded_stream, parse_constant=parse_constant)
...
class MyJSONParser(BaseParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        ...
        print str(stream)
        method =str(stream).split()[1]
        print method
        url = str(stream).split('/')[1]
        # raw=request["raw_body"]
        try:

            data = stream.read().decode(encoding)
            # print type(data),"cddcddcdcdd"
            data = json.loads(data)
            # print"sddsdssdsdsdsddssds", type(data), url
            if method == "POST":
                if url in ["staff", "admin", "patient", "corporatehr"] :
                    # print "rggrgrgr"
                    d = dict()
                    s = dict()
                    p = dict()
                    for i in data :
                        if i == "email":
                            s.update({i:data[i]})
                        elif i in ["gender","first_name","middle_name","last_name","Qualification","user_type","phone"]:
                            p.update({i: data[i]})
                        else:
                            d[i]=data[i]
                    # print d,p,s
